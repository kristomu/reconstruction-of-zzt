...
template<typename T> conf_array<T>::conf_array(T const* str)
	: low(1), high(strlen(str)), buf((T*)str) {}

template<typename T> conf_array<T>::conf_array(T ch)
	: low(1), high(1), buf(&ch) {}
...
template<size_t max_size>
inline string varying_string<max_size>::operator + (const char* str)
const {
	string result;
	size_t left_len = length();
	size_t right_len = strlen(str);
...
template<size_t max_size>
inline string varying_string<max_size>::operator + (unsigned char ch)
const {
	string result;
	size_t len = length();
	memcpy(result.body, body, len);
...
template<size_t max_size>
inline void str(unsigned long val, int width,
	varying_string<max_size> & s) {
...
template<size_t max_size>
inline void str(double val, int width, int prec,
	varying_string<max_size> & s) {
	s.set_length(sprintf(s.body, "%*.*f", width, prec, val));
}

inline string copy(varying_string_header const & str, integer p,
	integer n) {
	string result;
	size_t len = str.length();
	result.set_length(0);
...
template<size_t max_size>
inline void Delete(varying_string<max_size> & s, integer index,
	integer count) {
...
class matrix {
	public:
		...
		integer size()  const {
			return (high_1-low_1+1)*(high_2-low_2+1);
		}
