...
const integer PROMPT_ANY = 2;
        ...
        void SidebarPromptCharacter(boolean editable, integer x, integer y, TString50 prompt, byte& value);
        void SidebarPromptSlider(boolean editable, integer x, integer y, string prompt, byte& value);
        void SidebarPromptChoice(boolean editable, integer y, string prompt, string choiceStr, byte& result);
        void SidebarPromptDirection(boolean editable, integer y, string prompt, integer& deltaX, integer& deltaY);
        void PromptString(integer x, integer y, integer arrowColor, integer color, integer width, byte mode, TString50& buffer);
        boolean SidebarPromptYesNo(string message, boolean defaultReturn);
        void SidebarPromptString(string prompt, TString50 extension, string& filename, byte promptMode);
        void PauseOnError();
        ...
        void GameWorldSave(TString50 prompt, TString50& filename, TString50 extension);
        boolean GameWorldLoad(TString50 extension);
        void CopyStatDataToTextWindow(integer statId, TTextWindowState& state);
        void AddStat(integer tx, integer ty, byte element, integer color, integer tcycle, TStat template_);
        ...
        boolean BoardShoot(byte element, integer tx, integer ty, integer deltaX, integer deltaY, integer source);
        void CalcDirectionRnd(integer& deltaX, integer& deltaY);
        void CalcDirectionSeek(integer x, integer y, integer& deltaX, integer& deltaY);
...
const array<0 , 7,varying_string<5> > ProgressAnimStrings = 
        {{"....|", "...*/", "..*.-", ".*..\\", "*...|", "..../", "....-", "....\\"}};
const array<1 , 7,varying_string<8> > ColorNames = 
...
const varying_string<16> LineChars = "\371\320\322\272\265\274\273\271\306\310\311\314\315\312\313\316";
